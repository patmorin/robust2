...
\begin{proof}[Proof of \thmref{main}]
  ...
  Since $p\not\in F^+_P$, \clmref{root-explode} implies that there is a
  subset $X_{p'}\subseteq A_i'$, with $|X_{p'}|\ge (\epsilon/4)|A_i'|$
  such that, for every $x\in X_{p'}$, $G-F$ contains a path from $p$
  to $x$ of length at most $C\diam'(T_{a_i'})\le C\diam'(T_{a_i})$.
  ...
  \begin{align*}
    \dist_{G-F}(p,q) & \le \dist(p,p') + \dist_{G-F}(p',p'') +
      \dist(p'',q'') + \dist_{G-F}(q'',q') + \dist(q',q) \\
      & \le (2C+2)\diam'(a_i') + \dist(q',q) \\
      & \le (2C+3)\diam'(a_i') + \diam'(b_i) + \dist(p,q) \\
  \end{align*}
