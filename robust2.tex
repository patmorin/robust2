...
\begin{proof}
  ...
  apply induction on $T_{u_0}$.   This yields a set $X$ of size
  \[
  ...
  \begin{enumerate}
    ...
    \begin{enumerate}
      ...
      $\diam'(T_{u_0}) \le (1-1/2d)\diam'(T_u)$.  We can therefore apply
    ...
    \item $|F^+_{T_{u_0}}|> (1-\beta/\Delta)|T_{u_0}|$.  In this case,
     $F^+_T= L(T_{u_0})\cup F^+_{T_1}$, so $F^+_T= |T_{u_0}|
     + |F^+_{T_1}|$.  Therefore, $p\in L(T_1)$.  Now, we apply
     ...
     \begin{align*}
       |X| & \ge (1-a/\Delta)|(T_1)_u| - |F^+_{T_1}\cap L((T_1)_u)| \\
           ...
           & =   (1-a/\Delta)|(T_1)_u| - |F^+_{T}\cap L(T_u)| + |T_{u_0}| 
             & \text{(since $F^+_T= L(T_{u_0}) \cup |F^+_{T_1}|$)} \\
