...
\begin{enumerate}
  ...
  \begin{enumerate}
     \item then $F^+_T\gets F^+_T\cup S_{H_T}(F^+_{T_{u_0}})$. 
         \newline [Abandon points in $T_1$ can only reach $T_{u_0}$ through points abandoned in Step~2.]
...
\begin{clm}
  Let $C=4d$ and let $a=??$.  For any $F\subset L(T)$, there exists
  $F^+_T\supseteq F$ with $|F^+_T|\le (1+\epsilon)|F|$ such that, for
  every point $p\in L(T)\setminus F^+_T$, there exists $X\subset L(T)$,
  $|X|\ge (1-a/\Delta)|T|-|F\cap L(T)|$ such that for every $q\in X$,
  $G_T-F$ contains a path from $p$ to $q$ of length at most $C\diam(T)$,
  for $C\le 4d$.
\end{clm}

\begin{proof}
  The set $F^+_T$ is obtained from $F$ as follows.  We first compute a set 
  \[  
       F_0 = \cup\{ L(T_u) : |F\cap L(T_u)| \ge (1-\epsilon/(100\Delta))|T_u| \}
  \]
  It is not hard to see that $|F_0|\le (1+\epsilon/(50\Delta))|F|$.
  Next, we use \clmref{f-plus} to compute the set $F^+_T$ but
  with respect to the set $F_0$ and using the value $\epsilon/2$
  rather than $\epsilon$.  The result set has size $F^+_T\le
  (1+\epsilon/(50\Delta))(1+\epsilon/(2\Delta))|F| \le
  (1+\epsilon/\Delta)|F|$.

  The proof is by induction on $|T|$.  If $|T|\le\kappa$, the result is
  trivial since $G_T$ is the complete graph.  For $|T|>\kappa$, there
  are several cases to consider:
  \note{PM}{I'm stuck here. :(}
  \begin{enumerate}
    \item $|F^+_{T_{u_0}}|\le (1-\beta/\Delta)|T_{u_0}|$. In this case, there are two subcases
    to consider:
    \begin{enumerate}
      \item $p\in L(T_{u_0})$.  In this case, we know that $T_{u_0}$ is not $F$-dense since, if it were, it $F^+_T$ would include $p$.
...
\begin{align*}
  |X_0| & \ge (1-a/\Delta)|T_{u_0}|-|F\cap L(T_{u_0})| \\
        & \ge (1-a/\Delta)|T_{u_0}|-(1-\delta\rank(T_{u_0})/\Delta)|T_{u_0}| & \text{(since $T_{u_0}$ is not $F$-dense.)}\\
        & \ge (1-a/\Delta)|T_{u_0}|-(1-\delta/\Delta)|T_{u_0}| 
    & \text{(since $\rank(T_{u_0})\ge 1$ when $|T|\ge \kappa \ge 5$)} \\
        & = ((\delta-a)/\Delta)|T_{u_0}| \\
        & \ge ((\delta-a)/(3\Delta))|T| 
        & \text{(since $|T|/3\le |T_{u_0}|$).} 
\end{align*}
      By Property~(PR2) of $H_T$ (with $\zeta = (\delta-a)/3$ and $\eta = a$), we can then take $X=N_{H_T}(X_0)\setminus(F\cap L(T))$.  Then 
      \begin{align*}
         |X| & \ge (1-\eta/\Delta)|T|-|F\cap L(T)| \\
             & = (1-a/\Delta)|T|-|F\cap L(T)| \\
      ...
      edge from $p$ to some point $p'\in L(T_{u_0})\setminus F^+_{T_{u_0}}$.  As described
      in the previous case, there is a set $X$ of
      size $(1-a\rank(T)/\Delta)|T|-|F\cap L(T)|$ that is reachable from $p'$
    ...
    \item $|F^+_{T_{u_0}}|> (1-\beta/\Delta)|T_{u_0}|$.  In this case,
      ...
      Let $T_1'$ and $T_1''$ be the two subtrees of $T_1$ rooted at the children of the root.


      Now, we apply induction on $T_1$ and obtain a set $X_1$ that can be
      reached by $p$ in $G_T-F$ with paths of length at most $C\diam'(T_1)\le
      C\diam'(T)$. Observe that 
      \[ |F\cap L(T_1)| < (1-\delta\rank(T)/\Delta)|T_1| \]
      since, otherwise $T$ would be $F$-dense.
      Therefore,
     \begin{align*}
       |X_1| & \ge (1-a\rank(T_1)/\Delta)|T_1| - |F\cap L(T_1)| \\
           &  \ge (1-a\rank(T_1)/\Delta)|T_1| 
                - (1-\delta\rank(T)/\Delta)|T_1| \\
           &  \ge (1-a\rank(T_1)/\Delta)|T_1| 
                - (1-\delta\rank(T_1)/\Delta)|T_1|  \\
           &  \ge ((\delta-a)\rank(T_1)/\Delta)|T_1| \\
           &  \ge ((\delta-a)\rank(T)/(3\Delta))|T|
     \end{align*}
     Again, applying Property~(PR2) of $H_T$ shows that the set
     $X=N_{H_T}(X_1)$ satisfies the conditions of the claim.
     \note{PM}{Not correct, this gives a path of length $(C+1)\diam'(T)$.}
    \end{enumerate}
\end{proof}
